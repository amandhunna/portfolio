...
**Source: Layer 1 — Portfolio** (code implementation)
...
**Figma-side companion:** For UX/design workflows (grid setup, text styles, component variants, handoff), see **`.agent/figma-standards.md`**. Rules here are the code implementation mirror; keep both in sync when tokens change.
...
## Rule 9b — Visual Weight Balance

Balance hierarchy by size, contrast, saturation, and spacing. A small high-contrast element can outweigh a large muted one — use intentionally. One primary CTA per view; avoid multiple elements with equal visual weight.
